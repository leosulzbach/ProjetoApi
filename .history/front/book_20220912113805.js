...
const searchBook = async () => {
    ...
    categorie.forEach(async (data) => {
        if (filter === data.description) {
            const data = await axios.get(`${ENDPOINT}/books/?CategorieId=${data.id}`);
            table(data);
        }
    });
    books.forEach(async (book) => {
        if (filter === book.description) {
            const data = await axios.get(`${ENDPOINT}/books/?title=${filter}`);
            table(data);
        }
    })
